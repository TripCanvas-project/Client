  ...
  trips.forEach((trip) => {
    ...
    updateStatusBtn.addEventListener("click", (e) => {
        ...
        .forEach((status) => {
          ...
          btn.onclick = async (e) => {
            ...
            if (result) {
              ...
              const tripsInNewStatus = await fetchWithAuth(
                `${API_BASE}/trip?status=${newStatus}`
              );
      ...
      const closePalette = (event) => {
        if (
          !statusPalette.contains(event.target) &&
          event.target !== updateStatusBtn
        ) {
          statusPalette.remove();
...
async function loadMyChallenges() {
  try {
    const res = await fetchWithAuth(`${API_BASE}/bucketlist`);
    const bucketlists = res.bucketlists ?? [];

    // ✅ formatBucket 기준: status는 "completed" 또는 "in-progress"
    const completedCount = bucketlists.filter(
...
function renderChallenges(challenges) {
  ...
  challenges.forEach((challenge) => {
    const progressPercent = Math.min(
      Math.round((challenge.completedCount / challenge.targetCount) * 100),
      100
    );
    ...
    card.innerHTML = `
            <div class="challenge-icon" style="font-size: 24px;">${
              challenge.theme || "🎯"
            }</div>
...
document
  .getElementById("create-new-trip-btn")
  ?.addEventListener("click", async () => {
    try {
      ...
      if (response.ok) {
        ...
        // ✅ 핵심: planning 탭 버튼을 찾아서 클릭 이벤트를 발생시킴
        const planningTab = document.querySelector(
          '.tab-btn[data-tab="planning"]'
        );
        if (planningTab) {
          planningTab.click();
        }
