...
function initKakaoMap() {
    ...
    if (!mapContainer) {
        console.error(
            "카카오 지도를 표시할 요소를 찾을 수 없습니다: #kakao-map"
        );
        return;
    }
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    function updateSelectedStyles() {
        ...
        selectedChips.forEach((chip) => {
            const value =
                chip.getAttribute("data-value") || chip.textContent.trim();
  ...
  async function fetchCurrentTripStatus() {
    ...
    if (generatePlanButton) {
        generatePlanButton.addEventListener("click", async () => {
            ...
            if (status !== "planning") {
                alert(`현재 상태는 '${status}'입니다. planning 상태에서만 가능합니다.`);
                return;
            }
            ...
            const tripData = {
                ...
                detail_addr:
                    document.getElementById("sub-destination")?.value || "",
                ...
                place_themes:
                    document.getElementById("selected-styles")?.value || "",
    ...
    document.querySelectorAll(".panel-tabs .tab").forEach((tab) => {
        tab.addEventListener("click", () => {
            ...
            tab.classList.add("active");
            document
                .getElementById(`${panelName}-content`)
                ?.classList.add("active");
...
async function initCollaboration() {
    try {
        ...
        if (!tripId) {
            ...
            if (!currentTripId) {
                console.warn(
                    "⚠️ Trip ID not available, collaboration disabled"
                );
                return;
            }
        }
...
async function loadMyExpenses() {
    ...
    try {
        const response = await fetch(
            `${API_BASE_URL}/budget/my/${currentTripId}`,
            {
                headers: {
                    Authorization: `Bearer ${token}`,
                },
...
function openEditExpenseForm(expenseId) {
    ...
    const category = expenseItem
        ...
        ?.textContent.replace("#", "");
    const amountText =
        expenseItem.querySelector(".expense-amount")?.textContent;
